...
# Image1: nodal localization of the top 1% of PSS pairs for untransformed LQ.
# Run from any directory with: Rscript /path/to/images/Image1.R
...
script_path <- if (length(file_argument)) {
  ...
  normalizePath("Image1.R", mustWork = TRUE)
}
...
output_file <- file.path(image_dir, "Image1.png")
tail_name <- "top"
...
tail_indices <- ranking[seq_len(tail_size)]
tail_count <- tabulate(
...
tree_panel <- base_tree +
  ...
  labs(
    title = "Untransformed LQ: top 1% of PSS pairs",
    subtitle = paste0(
      Ntip(tree), " species · ", nrow(pairs), " available pairs · ",
      tail_size, " pairs in the top tail"
    ),
    caption = paste0(
      "Each pair is assigned once to its MRCA. Node colour is the top-tail ",
