...
#SBATCH --job-name=genphen-caas-pss-driver
#SBATCH --cpus-per-task=1
#SBATCH --mem=16G
#SBATCH --time=7-00:00:00
#SBATCH --partition=std-cpu
#SBATCH --output=logs/genphen-caas-pss-driver-%j.out
#SBATCH --error=logs/genphen-caas-pss-driver-%j.err
...
if [[ ! -f "${pipeline_dir}/run_pipeline.sh" || ! -f "${pipeline_dir}/main.nf" ]]; then
    echo "Submit this script from the approach.pss directory." >&2
...
echo "GenPhen CAAStools PSS Nextflow driver"
